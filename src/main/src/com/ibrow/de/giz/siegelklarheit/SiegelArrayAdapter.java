...
public class SiegelArrayAdapter extends ArrayAdapter<ShortSiegelInfo> {
	...
	public SiegelArrayAdapter(Context context, ShortSiegelInfo[] values) {
		...
		blankLogo = context.getResources().getDrawable(
				R.drawable.blank_label_logo);
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		View rowView = inflater
				.inflate(R.layout.listitem_siegel, parent, false);

		ShortSiegelInfo siegel = siegels[position];
		SiegelRating rating = siegel.getRating();

		ImageView rating_image_view = (ImageView) rowView
				.findViewById(R.id.rating_symbol_image);
		rating_image_view.setImageDrawable(resources.getDrawable(resources
				.getIdentifier("@drawable/" + rating.getImageIdentifier(),
						null, packageName)));

		TextView rating_color = (TextView) rowView
				.findViewById(R.id.rating_color);
		rating_color.setBackgroundColor(rating.getColor());

		TextView title_view = (TextView) rowView
				.findViewById(R.id.siegel_name_text);
		title_view.setText(siegel.getName());

		ImageView logo_image_view = (ImageView) rowView
				.findViewById(R.id.logo_view);
		...
		if (image != null) {
			...
			LoadSiegelLogoTask logo_task = new LoadSiegelLogoTask(
					logo_image_view, siegel);
			logo_task.execute(siegel);
		}

		switch (siegel.getRating()) {
		case NONE:
			TextView no_criteria_view = (TextView) rowView
					.findViewById(R.id.siegel_no_criteria_text);
			no_criteria_view.setText(R.string.criterion_none);
			LinearLayout critertia_view = (LinearLayout) rowView
					.findViewById(R.id.criteria_holder);
			critertia_view.setVisibility(View.GONE);
			no_criteria_view.setVisibility(View.VISIBLE);
			break;

		case UNKNOWN:
			TextView no_criteria_view2 = (TextView) rowView
					.findViewById(R.id.siegel_no_criteria_text);
			no_criteria_view2.setText(R.string.criterion_unknown);
			LinearLayout critertia_view2 = (LinearLayout) rowView
					.findViewById(R.id.criteria_holder);
		...
		default:
			...
			for (int i = 0; i < IdentifeyeAPIInterface.EXPECTED_CRITERIA_NUMBER; i++) {
				if (i < critertia_quantity) {
					...
					if (c != null) {
						id = i + 1;
						criteria_text = (TextView) rowView
								.findViewById(resources.getIdentifier(
										"@id/criteria_text_" + id, null,
										packageName));
						criteria_text.setText(resources.getText(resources
								.getIdentifier("@string/"
										+ c.getType().getNameIdentifier(),
										null, packageName)));

						criteria_image = (ImageView) rowView
								.findViewById(resources.getIdentifier(
										"@id/criteria_image_" + id, null,
										packageName));
						criteria_image
								.setImageDrawable(resources.getDrawable(resources
										.getIdentifier(
												"@drawable/"
														+ c.getValue()
																.getNameIdentifierForImage(),
												null, packageName)));
					} else {
